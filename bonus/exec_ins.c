...
t_num	*exec_instructions(t_num *nb, t_ins *ins)
{
	...
	while (elet)
	{
		if (!ft_strncmp(elet->str, "sa", 2) || !ft_strncmp(elet->str, "sb", 2)
			|| !ft_strncmp(elet->str, "ss", 2))
			go_switch(&stack_a, &stack_b, elet, ins);
		if (!ft_strncmp(elet->str, "pa", 2) || !ft_strncmp(elet->str, "pb", 2))
			go_push(&stack_a, &stack_b, elet, ins);
		if (!ft_strncmp(elet->str, "ra", 2) || !ft_strncmp(elet->str, "rb", 2)
			|| !ft_strncmp(elet->str, "rr", 2))
